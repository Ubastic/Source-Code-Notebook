...
def load_graph_AGC(dataset, k):
    ...
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        shape=(n, n), dtype=np.float32)
    adj_train = sp.csr_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        shape=(n, n), dtype=np.float32)
    ...
    return adj_train,adj


def load_graph_pretrain(dataset, k):
    if k:
        path = '../graph/{}{}_graph.txt'.format(dataset, k)
    else:
        path = '../graph/{}_graph.txt'.format(dataset)

    data = np.loadtxt('{}.txt'.format(dataset))
    n, _ = data.shape
...
if __name__ == '__main__':
    adj_train,adj = load_graph_pretrain('cite', None)
