...
def encode_onehot(labels):
    # class_label(2708)
    classes = set(labels)  # set() 函数创建一个无序不重复元素集 class_label中存在重复元素
    ...
    labels_onehot = np.array(list(map(classes_dict.get, labels)),  # get函数得到字典key对应的value
                             dtype=np.int32)    # (2708,7) 2708篇文章对应的类别one-hot向量
...
def load_data(path="../data/cora/", dataset="cora"):
    ...
    # idx_features_labels tuple形式(2708，1435)    <paper_id> <word_attributes>+ <class_label>
    # 如果去除paper_id 和 class_label 为Node Features在此数据集中为描述论文特点的特征向量
    idx_features_labels = np.genfromtxt("{}{}.content".format(path, dataset),
                                        dtype=np.dtype(str))
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)  # 储存为csr型稀疏矩阵(2708,1433)
    ...
    idx = np.array(idx_features_labels[:, 0], dtype=np.int32)   # (2708) paper_id的列表
    idx_map = {j: i for i, j in enumerate(idx)}     # 根据paper_id的列表统计每篇文章的出现次数
    ...
    # 图中边的信息(5429,2)一共有5429条边，2表达的是两个节点相连，存储的是papaer_id
